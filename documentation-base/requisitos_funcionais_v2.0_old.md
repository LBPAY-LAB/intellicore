## Este documento não deve ser considerado pelo claude code, agentes ou sub agentes.


O intelliCore é uma solução de gestão de Oraculos.

O objetivo é a implementação de soluções baseadas em Agentic RAGs e MCPs Servers conforme a seguinte organização hierarquica:


O intellicore pode ter uma ou mais soluções
Uma Solução Abstratcta tem 
   um ou mais Oráculos
      um Oráculo tem
         um RAG trimendicional (PostGres, NebulaGraph e PgVector)
            um ou mais pipelines de ingestão:
               Pipeline de campos do Oráculo:
                  Nome do Oráculo
                  Data de Criação
                  Flags que permitem classificar um Oraculo
                  Auto-Consciência: Texto que explica e caracterizar a essência de um Oraculo;
                  Permitir Update destes dados
               Pipeline de Upload de documentos (Pdf, md, txt, csv,...):
                  Registrar em Cada documento:
                     Data de entrada (timeStamp)
                     Quem deu entrada 
                     Versão
                     flags que o caracterizam e classificam o documento,
                     permitir posteriormente revogá-lo e subsitui-lo por outro mais recente:
               Pipeline de WebScrapping de sites;
               Pipeline de Fecth de APIs;
               Pipeline de Consultas a MCPs:
         zero ou mais objetos do tipo:
            pode ser chamadas a APIs;
            pode ser estruturas de dados;
         zero ou mais agentes
            agentes locais em Skills
            agentes remotos
            

      Continuar a descrever o que se pretende do intellicore.
...
- Objetos. Cada objeto representa uma estrutura de dados. Pode ser JSON, XML, uma tabela de base de dados;...
- Skills de Agentes
- Fluxos customizados
...
#### RF005: Consulta ao Conhecimento via Agentic RAG Trimodal
**Descrição**: Multi-agent system consulta Oráculo usando padrões Agentic RAG para queries inteligentes, auto-corretivas e com validação de qualidade
...
- **Fusion Agent (CrewAI)**: Combina resultados das 3 fontes, resolve conflitos, mantém attribution
- **Generator Agent (CrewAI)**: Sintetiza resposta final usando contexto multi-source
- **Reflector Agent (CrewAI)**: Valida grounding da resposta, detecta hallucinations, auto-corrige

**Padrões Agentic RAG Implementados**:
1. **Routing Pattern**: Router Agent decide dinamicamente SQL vs Graph vs Vector baseado em análise da query
2. **CRAG (Corrective RAG)**: Fusion Agent avalia qualidade do contexto, triggers re-query com prompt refinado se insuficiente (até 3 tentativas)
3. **Self-RAG**: Reflector Agent verifica se resposta está grounded nos documentos recuperados, auto-corrige se hallucination detectada
4. **Multi-Source Fusion**: Combina SQL (structured data) + Graph (relations) + Vector (semantic) em resposta única coesa
5. **Iterative Refinement**: Temporal Workflow orquestra loops de retry com checkpoints (durable execution)

**Orquestração com Temporal Workflow**:
- Pipeline durable: Router → Retrieval (paralelo 3×) → Fusion → Generation → Reflection
- Checkpoints após cada fase (resiliente a falhas)
- Retry logic automático (max 3 tentativas por fase)
- Long-running queries suportadas (até 10 minutos)
- Human-in-the-loop opcional (aprovações manuais)

**Critérios de Aceitação**:
- ✅ Router Agent classifica corretamente 90%+ das queries (SQL vs Graph vs Vector)
- ✅ Retrieval Agents retornam resultados relevantes em <2s (p95)
- ✅ Fusion Agent combina 3 fontes sem perda de informação (attribution preservada)
- ✅ Generator Agent sintetiza resposta coesa com contexto completo
- ✅ Reflector Agent detecta 95%+ das hallucinations (validação via grounding check)
- ✅ CRAG pattern triggers re-query quando contexto insuficiente (max 3 tentativas)
- ✅ Temporal Workflow executa pipeline completo end-to-end com checkpoints
- ✅ Rastreabilidade de fonte para cada trecho da resposta (SQL vs Graph vs Vector)
- ✅ Multi-idioma suportado (português, inglês, espanhol)
- ✅ Latência total <3s (p95) - incluindo routing, retrieval, fusion, generation, reflection

**Performance**:
- Router Agent: <200ms (decisão SQL/Graph/Vector)
- Retrieval Agents (paralelo): <2s (p95) - 3 queries simultâneas
- Fusion Agent: <500ms (merge de resultados)
- Generator Agent: <1s (síntese LLM)
- Reflector Agent: <300ms (grounding check)
- **Total pipeline**: <3s (p95)

**Memória e Contexto**:
- Redis: Short-term memory (última query, contexto conversacional)
- PostgreSQL: Long-term memory (histórico de queries, feedback, ratings)
- Agent memory compartilhada via Temporal Workflow state

**Status v1**: Implementado (Traditional RAG - linear, single-source)
**Status v2.0**: **NOVO - Agentic RAG** (cyclical, multi-source, proactive reasoning, error correction)
