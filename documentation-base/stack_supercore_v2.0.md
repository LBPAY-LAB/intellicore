...
1. **Temporal Workflow v1.23+** (NOVO - Substitui Celery)
   - Durable execution engine para workflows complexos
   - SAGA pattern built-in com compensações automáticas
   - Long-running tasks (horas/dias sem bloquear workers)
   - Observability superior (Temporal UI, workflow replay)
   - Human-in-the-loop (signals, queries)
   - Multi-tenancy via task queues isoladas
   - Polyglot: SDKs Go + Python

2. **Apache Pulsar v3.4.0** (NOVO)
   - Substitui Kafka para messaging
   - Multi-tenancy nativo
   - Schema Registry com Pydantic
   - Geo-replication

3. **CrewAI + LangGraph** (NOVO)
...
│ Temporal Workflow 1.23 (Durable Workflows) + Apache Pulsar 3.4  │
│ Interaction Broker (Go) + Template System (Pydantic)            │
...
- Temporal Workflow: Workflows duráveis (10k+ workflows/sec)
...
##### Temporal Workflow v1.23+

**Papel**: Durable execution engine para workflows complexos, long-running tasks e SAGA patterns

**Versão Mínima**: 1.23.0

**Por que Temporal?**

Temporal substitui Celery (usado em v1.0) com capacidades avançadas de orquestração:

1. **Durable Execution**: Workflows sobrevivem a crashes de workers (estado persiste automaticamente)
2. **SAGA Pattern Built-in**: Compensações automáticas para transações distribuídas
3. **Long-Running Tasks**: Tasks podem pausar por horas/dias/semanas sem bloquear workers
4. **Observability Superior**: Temporal UI mostra histórico completo de execução
5. **Workflow Replay**: Debug workflows re-executando history events (determinístico)
6. **Human-in-the-Loop**: Signals e Queries permitem interação humana com workflows em execução
7. **Multi-Tenancy**: Task queues isoladas por solução (resource isolation)
8. **Polyglot**: SDKs oficiais para Go e Python (integração perfeita com stack SuperCore)
...
│                                                             │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ Persistence: PostgreSQL + Elasticsearch                 │ │
│ └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
                              ↕
┌─────────────────────────────────────────────────────────────┐
│ Workers (by Language & Solution)                            │
│ ┌──────────────────────┬────────────────────────────────┐   │
│ │ Go Workers           │ Python Workers                 │   │
│ │ ├─ CRUD operations   │ ├─ RAG pipelines               │   │
│ │ ├─ Notifications     │ ├─ Document processing         │   │
│ │ ├─ Sync jobs         │ ├─ AI agent execution          │   │
│ │ └─ Integrations      │ └─ LLM workflows               │   │
│ └──────────────────────┴────────────────────────────────┘   │
│                                                             │
│ Task Queues (Multi-Tenant Isolation):                      │
│ ├─ solution-{solution_id}      (dedicated per solution)    │
│ ├─ global-crud                 (shared CRUD operations)    │
│ └─ global-ai                   (shared AI workers)         │
└─────────────────────────────────────────────────────────────┘
```

**Conceitos Core**:

1. **Workflow**: Durable function representing business process
   - Survives worker crashes, restarts, deployments
   - Deterministic execution (same inputs = same outputs)
   - Event sourcing for state persistence

2. **Activity**: Individual unit of work (can fail/retry independently)
   - Non-deterministic operations (DB writes, API calls)
   - Configurable retry policies
   - Timeouts and heartbeats

3. **Task Queue**: Worker pool routing mechanism
   - Multi-tenant isolation (1 queue per solution)
   - Priority-based routing
   - Dynamic worker scaling

4. **Signal**: External event sent to running workflow
   - Human approvals (QA, production deploy)
   - Cancel/pause/resume operations
   - Configuration updates

5. **Query**: Synchronous request to workflow state
   - Progress reporting
   - Status checks
   - Live debugging

**Temporal vs Celery Comparison**:

| Feature | Celery (v1.0) | Temporal (v2.0) |
|---------|--------------|----------------|
| **State Management** | Manual (DB/Redis) | Automatic (event sourcing) |
| **SAGA Pattern** | Manual compensation | Built-in compensation |
| **Long-Running Tasks** | ❌ Blocks workers | ✅ Sleep/await (no blocking) |
| **Observability** | Flower (basic) | Temporal UI (complete history) |
| **Debugging** | Logs only | Workflow replay (deterministic) |
| **Human-in-the-Loop** | Manual polling | Signals/Queries (native) |
| **Multi-Tenancy** | Manual routing | Task queues (built-in) |
| **Polyglot** | Python-only | Go + Python SDKs |
| **Retries** | Per-task config | Per-activity + workflow-level |
| **Versioning** | Manual migration | Workflow versioning built-in |

**Use Cases no SuperCore v2.0**:

1. **CreateSolution Workflow** (SAGA Pattern):
   ```python
   # Transação distribuída com compensação automática
   @workflow.defn
   class CreateSolutionWorkflow:
       @workflow.run
       async def run(self, request: CreateSolutionRequest) -> Solution:
           # Activity 1: Create solution record
           ...
           try:
               rag_global = await workflow.execute_activity(
           ...
           # Activity 3: Process documents (long-running, 30 min)
           if request.documents:
               await workflow.execute_activity(
                   process_documents,
                   ProcessDocRequest(
                       oracle_id=rag_global.id,
...
2. **ProcessDocument Workflow** (Long-Running + Progress Tracking):
   ```python
   @workflow.defn
   class ProcessDocumentWorkflow:
       ...
       @workflow.run
       async def run(self, doc: Document) -> ProcessResult:
           # Step 1: Extract text (pode levar minutos)
           self._progress = 10
           ...
           chunks = await workflow.execute_activity(
               chunk_text,
               ChunkRequest(text=text, strategy="semantic"),
               start_to_close_timeout=timedelta(minutes=2)
           )

           # Step 3: Generate embeddings (paralelo)
           self._progress = 50
           embed_futures = []
           for chunk in chunks:
           ...
           embeddings = await asyncio.gather(*embed_futures)

           # Step 4: Index in vector DB
           self._progress = 80
           await workflow.execute_activity(
               index_embeddings,
               IndexRequest(oracle_id=doc.oracle_id, embeddings=embeddings),
               start_to_close_timeout=timedelta(minutes=5)
           )

           self._progress = 100
           return ProcessResult(status="completed", chunks_processed=len(chunks))

       @workflow.query
       def get_progress(self) -> int:
           """Query para frontend mostrar progresso em tempo real"""
           return self._progress
   ```

3. **AgentExecution Workflow** (Human-in-the-Loop):
   ```python
   @workflow.defn
   class AgentExecutionWorkflow:
       ...
       async def run(self, agent_request: AgentRequest) -> AgentResult:
           ...
           if agent_request.requires_approval:
               # Wait for approval signal (pode levar horas/dias)
           ...
           return AgentResult(status="deployed", result=deploy_result)

       @workflow.signal
       def approve(self):
           """Signal enviado por humano para aprovar"""
           self._approved = True

       @workflow.signal
       def reject(self):
           """Signal para rejeitar e cancelar workflow"""
           workflow.continue_as_new(status="rejected")
   ```

**Multi-Tenancy Strategy**:

Cada **Solution** tem task queue dedicada para isolamento completo:

```python
# Start workflow for specific solution (isolated worker pool)
await client.start_workflow(
    CreateSolutionWorkflow.run,
    request,
    id=f"create-solution-{solution_id}",
    task_queue=f"solution-{solution_id}",  # Dedicated workers
    execution_timeout=timedelta(hours=2)
)
```

**Worker Deployment** (Polyglot):

```yaml
# docker-compose.yml
services:
  # Go Workers (CRUD, sync, notifications)
  temporal-worker-go:
    build: ./workers/go
    environment:
      TEMPORAL_ADDRESS: temporal:7233
      TASK_QUEUE: global-crud
    deploy:
      replicas: 3

  # Python Workers (RAG, AI, document processing)
  temporal-worker-python:
    build: ./workers/python
    environment:
      TEMPORAL_ADDRESS: temporal:7233
      TASK_QUEUE: global-ai
    deploy:
      replicas: 5

  # Solution-specific workers (auto-scaled)
  temporal-worker-solution:
    build: ./workers/solution
    environment:
      TEMPORAL_ADDRESS: temporal:7233
      TASK_QUEUE: solution-${SOLUTION_ID}
    deploy:
      mode: replicated
      replicas: 2
```

**Temporal Server Setup** (Docker Compose para dev):
...
- **Python**: `temporalio` (official SDK)
  ```bash
  pip install temporalio
  ```

- **Go**: `go.temporal.io/sdk` (official SDK)
  ```bash
...
- **Temporal UI**: http://localhost:8088
  - Workflow execution history (complete event log)
  - Workflow replay for debugging
  - Search workflows by ID, type, status
...
- **Throughput**: 10,000+ workflows/sec (cluster)
- **Latency**: < 100ms workflow start
- **Activity Latency**: < 10ms activity dispatch
- **Scalability**: Millions of concurrent workflows
- **History Size**: Up to 50MB per workflow (configurable)

**Integration with SuperCore Stack**:
...
async def main():
    ...
    # Create worker listening on multiple task queues
    worker = Worker(
        client,
        task_queue="global-ai",
        workflows=[
...
spec:
  ...
  template:
    spec:
      containers:
      - name: worker
        image: supercore/temporal-worker-python:latest
        env:
...
**Redis Complementar** (Retained for Caching):

Temporal **substitui Celery** para workflows, mas **Redis permanece** para:
- **Cache**: Instances, configurations, sessions
- **Pub/Sub**: Real-time notifications (WebSocket broker)
- **Rate Limiting**: API rate limits (token bucket)

Redis **NÃO é mais usado** para:
- ❌ Background job queue (agora Temporal workflows)
- ❌ Task result storage (Temporal event history)
- ❌ Task retry logic (Temporal retry policies)

**Migration Path (Celery → Temporal)**:

1. **Phase 1**: Run Celery + Temporal em paralelo
2. **Phase 2**: Migrate workflows one-by-one (start with CreateSolution)
3. **Phase 3**: Sunset Celery workers
4. **Phase 4**: Remove Celery dependencies

**ADR Reference**: Ver `arquitetura_supercore_v2.0.md` - ADR-008: "Temporal Workflow para Orquestração Durável"
