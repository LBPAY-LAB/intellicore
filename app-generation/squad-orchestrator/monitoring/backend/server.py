...
class BootstrapController:
    ...
    async def start_bootstrap(self, request: BootstrapRequest) -> BootstrapStatus:
        ...
        # 3. Limpar database do portal (eventos, cards, sessions antigas)
        monitoring_db = self.base_dir / "monitoring" / "data" / "monitoring.db"
        if monitoring_db.exists():
            import sqlite3
            conn = sqlite3.connect(monitoring_db)
            cursor = conn.cursor()

            # Limpar eventos históricos
            ...
            conn.commit()
            conn.close()
            print(f"   ✅ Limpou DB: {events_deleted} eventos, {cards_deleted} cards, {sessions_deleted} sessions")
        ...
        artefactos_dir = self.base_dir.parent.parent / "artefactos_implementacao"
        if artefactos_dir.exists():
            import shutil
            # Limpar cada squad folder
            for squad_dir in ["produto", "arquitetura", "engenharia", "qa", "deploy"]:
                squad_path = artefactos_dir / squad_dir
                if squad_path.exists():
                    shutil.rmtree(squad_path)
                    squad_path.mkdir(parents=True, exist_ok=True)
                    print(f"   ✅ Limpo: artefactos_implementacao/{squad_dir}/")
