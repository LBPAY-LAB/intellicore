...
#### RF062: Assistente IA Conversacional (Interface User ↔ LLM/RAG)
**Descrição**: Interface conversacional que permite ao usuário interagir com Oráculos via chat em linguagem natural, com streaming de respostas em tempo real.

**Contexto**: O Assistente IA é a interface principal para usuários finais consultarem o conhecimento do Oráculo. Diferente de APIs tradicionais, oferece experiência conversacional com contexto multi-turn e respostas geradas dinamicamente.

**Funcionalidades**:
- **Chat Interface**: UI tipo ChatGPT integrada ao Super Portal
- **Streaming em Tempo Real**: Respostas streamadas via Server-Sent Events (SSE) ou WebSocket
- **Context-Aware**: Mantém histórico de conversação para contexto multi-turn
- **Multi-Modal Input**: Aceita texto, imagens, documentos (PDF, DOCX)
- **Intent Routing Automático**: Roteia query para RAG search, SQL query, ou AI Agent conforme intent detectado
- **Query Reformulation**: LLM reformula query do usuário para otimizar retrieval
- **Citation & Sources**: Respostas incluem citações e links para documentos fonte

**Arquitetura**:
```
User → Chat UI → Interaction Broker → Intent Classifier (LLM)
                                    ├─→ RAG Pipeline (LangFlow) → Vector Search → LLM Synthesis
                                    ├─→ SQL Analyst (LangChain Agent) → PostgreSQL Query
                                    └─→ AI Agent (CrewAI) → Custom Tools
```

**Fluxo de Exemplo**:
1. **User Query**: "Quais são os requisitos de autenticação no RF045?"
2. **Intent Classification**: LLM classifica como "RAG search" (documentação)
3. **RAG Pipeline** (LangFlow):
   - Query reformulation: "authentication requirements RF045"
   - Semantic search em pgvector (documentação embeddings)
   - Top-K retrieval: 5 chunks mais relevantes
4. **LLM Synthesis**: GPT-4 sintetiza resposta usando chunks + prompt template
5. **Streaming Response**: Tokens streamados via SSE/WebSocket
6. **Context Persistence**: Histórico salvo em Redis para próximo turn

**Integrações**:
- **LangFlow**: Query router + RAG pipeline visual
- **LangChain**: Agents para SQL, APIs externas
- **CrewAI**: Multi-agent workflows quando necessário
- **Redis**: Session state + conversation history
- **PostgreSQL + pgvector**: Document storage + semantic search
- **WebSocket/SSE**: Real-time streaming

**Critérios de Aceitação**:
- **Performance**: Latência < 100ms para first token, < 2s para resposta completa (p95)
- **Streaming**: Suporte a SSE e WebSocket com reconnection handling
- **Context Window**: Mantém últimas 10 mensagens de conversação
- **Multi-Modal**: Upload de documentos (PDF, DOCX, TXT) para análise
- **Intent Accuracy**: ≥95% de acurácia em routing (RAG vs SQL vs Agent)
- **Citations**: 100% das respostas RAG incluem citações com links
- **Concurrency**: Suporte a 100+ conversações simultâneas por Oráculo
- **State Management**: Histórico persistido em Redis com TTL configurável

**Segurança**:
- Autenticação JWT obrigatória
- Rate limiting: 100 queries/min por usuário
- Input sanitization (prevenir prompt injection)
- Output moderation (prevenir conteúdo inadequado)
- Audit logging de todas as queries

**Casos de Uso**:
1. **Consulta de Documentação**: "Como configurar autenticação OAuth2?"
2. **Análise de Dados**: "Quantas transações foram processadas ontem?"
3. **Troubleshooting**: "Por que o workflow X falhou?"
4. **Onboarding**: "Quais são os primeiros passos para criar um Oráculo?"

**Tecnologias**:
- **Frontend**: React + WebSocket Client + Markdown Renderer
- **Backend**: FastAPI + SSE/WebSocket Server
- **LLM**: GPT-4 (synthesis), GPT-3.5-turbo (intent classification)
- **Orchestration**: LangFlow (RAG pipeline) + LangChain (agents)
- **State**: Redis (session + conversation history)
- **Search**: pgvector (semantic search)

**Métricas de Sucesso**:
- **Latência p50**: < 500ms (first token)
- **Latência p95**: < 2s (resposta completa)
- **Satisfação**: ≥4.5/5 (user feedback)
- **Resolução**: ≥80% de queries respondidas sem escalação humana
- **Uptime**: 99.9% (SLA)

**Dependências**:
- RF019: Geração Automática de Workflows LangFlow (RAG pipelines)
- RF020: Gerenciamento de Agentes (AI Agents disponíveis)
- RF030: MCP Server (ferramentas para agentes)
- RF045: Autenticação e Autorização (JWT, permissions)

**Fases de Implementação**:
- **Fase 1** (Q1 2025): Chat UI + RAG básico (texto)
- **Fase 2** (Q2 2025): Intent routing + Multi-modal (PDF, DOCX)
- **Fase 3** (Q3 2025): SQL Analyst + AI Agent integration
- **Fase 4** (Q4 2025): Advanced features (query reformulation, citation ranking)

**Status v1**: Não implementado
**Status v2.0**: Novo - Crítico para UX do Super Portal (Fundação em Fase 1)
