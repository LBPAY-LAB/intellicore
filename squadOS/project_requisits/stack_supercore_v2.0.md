...
## 5.6 ASSISTENTE IA CONVERSACIONAL

**Papel**: Interface conversacional em tempo real para usuários interagirem com Oráculos via chat

**Contexto**: O Assistente IA é a camada de apresentação principal do SuperCore v2.0, oferecendo experiência tipo ChatGPT para consultar conhecimento, executar workflows e interagir com agentes.

### 5.6.1 Arquitetura de Streaming
...
┌─────────────────────────────────────────────────────────────┐
│ Frontend (React)                                            │
│  - Chat UI (Markdown + Code Highlighting)                  │
...
│  - Interaction Broker (intent routing)                      │
...
└──────────────────┬──────────────────────────────────────────┘
   ...
   ┌────────┐ ┌────────┐ ┌────────┐
   │ RAG    │ │ SQL    │ │ Agent  │
   │Pipeline│ │Analyst │ │Executor│
   └────────┘ └────────┘ └────────┘
   (LangFlow) (LangChain)(CrewAI)
```
...
1. **Server-Sent Events (SSE)**:
   ...
   - **Uso**: Respostas LLM streaming

2. **WebSocket**:
   - Bidirecional (server ↔ client)
   - Baixa latência
   - Stateful connection
   - **Uso**: Chat interativo, real-time collaboration

**Decisão**: Suportar ambos (SSE default, WebSocket opcional)

### 5.6.2 Stack Frontend
...
function ChatAssistant({ oracleId }: { oracleId: string }) {
  ...
  const { data: streamChunk, error, readyState } = useEventSource(
    `/api/v1/oracles/${oracleId}/chat/stream`,
    { enabled: isStreaming }
  )
  ...
  useEffect(() => {
    if (streamChunk) {
      setMessages(prev => {
        ...
        if (lastMsg?.role === 'assistant' && !lastMsg.citations) {
          // Append to existing assistant message
          return [
            ...prev.slice(0, -1),
            { ...lastMsg, content: lastMsg.content + streamChunk }
          ]
        } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Send to backend (SSE will handle response)
    await fetch(`/api/v1/oracles/${oracleId}/chat`, {
      method: 'POST',
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map(msg => (
          <Card key={msg.id} className={msg.role === 'user' ? 'bg-blue-50' : 'bg-gray-50'}>
            <ReactMarkdown
              components={{
            ...
            {msg.citations && (
              <div className="mt-2 text-sm text-gray-600">
                ...
                {msg.citations.map(c => (
                  <a key={c.url} href={c.url} className="block underline">
                    {c.title}
                  </a>
                ))}
              </div>
...
|------------|--------|-----------|
| **react-markdown** | 9.0+ | Renderizar markdown das respostas |
| **react-syntax-highlighter** | 15.5+ | Syntax highlighting para código |
| **@microsoft/fetch-event-source** | 2.0+ | SSE client robusto |
| **zustand** | 4.5+ | State management (session, messages) |
| **date-fns** | 3.0+ | Formatação de timestamps |
| **lucide-react** | 0.300+ | Ícones (Send, Loader, AlertCircle) |
...
from services.interaction_broker import InteractionBroker
from services.session_manager import SessionManager
from core.auth import get_current_user

router = APIRouter(prefix="/api/v1/oracles/{oracle_id}/chat")
...
async def chat(
    ...
    conversation_id = request.conversation_id or session_mgr.create_session(
        oracle_id=oracle_id,
        user_id=user.id
    )
    ...
    broker = InteractionBroker(oracle_id=oracle_id)

    if request.stream:
        return StreamingResponse(
            stream_response(broker, conversation_id, request.message),
            media_type="text/event-stream"
        )
    else:
        response = await broker.handle_query(
...
```python
# services/interaction_broker.py
from typing import AsyncGenerator
...
class InteractionBroker:
    ...
    - RAG: Documentação, FAQs
    - SQL: Análise de dados, queries estruturadas
    - Agent: Workflows complexos, multi-step tasks
    """
    ...
    def __init__(self, oracle_id: str):
        ...
        self.intent_classifier = ChatPromptTemplate.from_messages([
            ("system", """Classify user query intent into ONE of:
            - RAG: Documentation lookup, definitions, how-to questions
            - SQL: Data analysis, statistics, reporting queries
            - AGENT: Complex workflows, multi-step tasks, integrations

            Respond with ONLY: RAG, SQL, or AGENT"""),
            ("user", "{query}")
        ])
    ...
    async def handle_query_stream(
        ...
        """Route query and stream response"""

        # 1. Classify intent
        intent_chain = self.intent_classifier | self.llm
        intent_result = await intent_chain.ainvoke({"query": query})
        intent = intent_result.content.strip().upper()

        # 2. Route to handler
        if intent == "RAG":
            handler = RAGPipelineHandler(self.oracle_id)
        elif intent == "SQL":
            handler = SQLAnalystHandler(self.oracle_id)
        elif intent == "AGENT":
            handler = AgentExecutorHandler(self.oracle_id)
        else:
            # Fallback to RAG
            handler = RAGPipelineHandler(self.oracle_id)

        # 3. Stream response
        async for chunk in handler.stream_response(query, conversation_id):
            yield chunk
```
...
class SessionManager:
    ...
    def create_session(self, oracle_id: str, user_id: str) -> str:
        """Create new conversation session"""
        conversation_id = f"conv:{oracle_id}:{user_id}:{uuid.uuid4()}"
    ...
    async def add_message(
        ...
        session_str = await self.redis.get(conversation_id)
        if not session_str:
        ...
        # Keep only last 20 messages (context window)
...
### 5.6.4 Stack de Infraestrutura

**Comunicação Real-Time**:

| Componente | Tecnologia | Propósito |
|------------|------------|-----------|
| **SSE Server** | FastAPI StreamingResponse | Stream LLM tokens |
| **WebSocket** | FastAPI WebSocket (fallback) | Chat bidirecional |
| **Session Store** | Redis 7.2+ | Conversation history |
...
┌─────────────────────────────────────┐
│ FastAPI Instances (3+)              │
│  - Sticky sessions (conversation_id)│
...
- **First Token**: < 100ms (p50), < 200ms (p95)
- **Complete Response**: < 2s (p50), < 5s (p95)
- **Concurrent Conversations**: 1000+ per instance
- **Message Throughput**: 10,000 msg/min

### 5.6.5 Integração com LangFlow

**LangFlow como RAG Pipeline**:

```python
# handlers/rag_pipeline_handler.py
...
class RAGPipelineHandler:
    def __init__(self, oracle_id: str):
        ...
        # Load LangFlow RAG pipeline
        self.flow = load_flow_from_json(
            f"flows/{oracle_id}/rag_pipeline.json"
        )
    ...
    async def stream_response(
        ...
        """Execute RAG pipeline and stream tokens"""

        # Get conversation history for context
        session_mgr = SessionManager()
        history = await session_mgr.get_history(conversation_id, limit=5)

        # Execute flow with streaming
        async for chunk in self.flow.stream({
            "query": query,
            "history": [msg.dict() for msg in history],
            "oracle_id": self.oracle_id
        }):
            if chunk.type == "token":
                yield StreamChunk(type="token", content=chunk.data)

            elif chunk.type == "citation":
                yield StreamChunk(type="citation", citation=chunk.data)
```

**LangFlow Pipeline Structure**:
...
Reranking (Cohere)
    ↓
LLM Synthesis (GPT-4) → Stream Tokens
    ↓
Citation Extraction
    ↓
...
- Redis 7.2+ (session storage)
- LangFlow 1.0+ (RAG pipeline builder)
