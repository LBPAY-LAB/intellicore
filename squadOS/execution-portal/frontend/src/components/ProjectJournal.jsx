...
function ProjectJournal() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchJournal()
      fetchBootstrapStatus()
      setCurrentTime(new Date()) // Update current time for runtime calculation
  ...
  const formatRuntime = () => {
    if (!bootstrapStatus?.started_at) return null

    const startDate = new Date(bootstrapStatus.started_at)
    ...
    if (diffDays > 0) {
      return `${diffDays}d ${remainingHours}h ${remainingMins}min`
    }
    if (diffHours > 0) {
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg p-6 border border-slate-700">
      ...
      <div className="mb-6">
        ...
        {bootstrapStatus?.started_at && (
          <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-lg p-4 border border-blue-700/50 mb-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Project Start Time */}
              <div className="flex items-center gap-3">
                <div className="text-3xl">🚀</div>
                <div>
                  <div className="text-xs text-slate-400 font-semibold">Projeto Iniciado em:</div>
                  <div className="text-sm text-slate-100 font-medium">
                    {new Date(bootstrapStatus.started_at).toLocaleString('pt-BR', {
                      day: '2-digit',
                      month: '2-digit',
                      year: 'numeric',
                      hour: '2-digit',
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-slate-400 font-semibold">Tempo de Execução:</div>
                  ...
                  {bootstrapStatus.status === 'stopped' && (
